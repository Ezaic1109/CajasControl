...
document.getElementById("btnGuardarDatos").addEventListener("click", function () {
    ...
    let nombrePrincipal = datos[0]?.nombre || "producto";
    nombrePrincipal = nombrePrincipal.replace(/\s+/g, "_"); // reemplaza espacios por guiones bajos
    ...
    const clave = `${nombrePrincipal}_${fechaHora}`;

    // Confirmar antes de guardar
    if (confirm("¿Deseas guardar la tabla con el ID: " + nombrePrincipal + "?")) {
...
document.getElementById("btnGuardarDatos").addEventListener("click", function () {
    ...
    let nombrePrincipal = datos[0]?.nombre || "producto";
    nombrePrincipal = nombrePrincipal.replace(/\s+/g, "_");
    ...
    if (confirm(`¿Deseas guardar como: ${clave}?`)) {
        localStorage.setItem(clave, JSON.stringify(datosTotales));
        alert("✅ Guardado con ID: " + clave);
    } else {
        alert("❌ Operación cancelada");
    }
});